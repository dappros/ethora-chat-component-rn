...
describe('regional locales render translated UI, not English', () => {
  ...
  it('fr-CA renders the Attach sheet in French', async () => {
    ...
    expect(text).toContain('Joindre');
    expect(text).toContain('Prendre une photo');
    expect(text).toContain('Photo ou vidéo');
    expect(text).toContain('Annuler');

    // The exact literals QA saw stuck in English.
    expect(text).not.toContain('Take photo');
    expect(text).not.toContain('Photo or video');
  ...
  it('es-US renders the Attach sheet in Spanish', async () => {
    ...
    expect(text).toContain('Adjuntar');
    expect(text).toContain('Tomar foto');
    expect(text).toContain('Foto o video');
