...
describe('i18n string table coverage', () => {
  ...
  it.each(translationFixKeys)('key "%s" exists in every locale', (key) => {
    for (const locale of locales) {
      // Keys are flat dotted strings (e.g. "attach.uploadFile"), not nested
  ...
  // None of these keys' English text is a word the other locales happen
  // to share, so a same-as-English value really does mean "translator
  // forgot this locale" rather than a legitimate coincidence.
  const mustDifferFromEnglish = translationFixKeys;
