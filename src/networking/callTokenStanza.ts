...
// Hermes has no global `atob` and RN ships no Buffer polyfill by default,
// so the JWT payload is decoded here by hand. Only used on the ~500-byte
// middle segment of a call token, so a table-driven loop is plenty fast
// and saves pulling a base64 dependency into the SDK.
const B64_ALPHABET =
...
// A JWT payload is UTF-8, and decodeBase64 hands back one char per byte.
// Re-interpret those bytes as UTF-8 so non-ASCII caller names (accents,
// Cyrillic) survive instead of turning into mojibake on the ring screen.
...
const decodeJwtPayload = (token: string): Record<string, any> | null => {
  ...
  try {
    ...
    return JSON.parse(bytesToUtf8(decodeBase64(padded))) as Record<string, any>;
  } catch {
    return null;
  }
};
...
const unwrapMucsub = (stanza: Element): Element => {
  // The web SDK also passes the pubsub#event xmlns here, but the ltx
  // typings bundled with the RN toolchain only declare getChild(name).
  // Matching on the element name alone is enough: the items > item >
  // message chain below fails fast on any other <event>.
  const inner = stanza
    ?.getChild?.('event')
    ?.getChild?.('items')
    ?.getChild?.('item')
...
const findCallData = (stanza: Element): Element | null => {
  // Direct `<data type="call-*">` on the message. Optional-called for the
  // same reason as onCallTokenMessage's name check: not every stanza that
  // reaches handleStanza is a real ltx Element.
...
const handleCallStateData = (
  ...
  const state = String(
    data.attrs?.state || data.attrs?.event || ''
  ).toLowerCase();
  ...
  const callIdMatches =
    !!stanzaCallId &&
    !!currentCall.callId &&
    currentCall.callId === stanzaCallId;
...
export const onCallTokenMessage = (stanza: Element): boolean => {
  // The web SDK calls `stanza.is('message')`. Here we read `.name`
  // directly: handleStanza is also fed plain stanza-shaped objects (test
  // fixtures, and the mucsub inner payloads we synthesize below), and
  // `.is` only exists on real ltx Elements. Name equality is what `.is`
  // checks anyway for a stanza with no explicit xmlns.
  ...
  if (data) {
    ...
    if (dataType === 'call-state') {
      ...
      try {
        pushLog('xmpp', '[call-state] received', {
...
export type CallSignalState = 'ended' | 'declined' | 'cancelled' | 'rejected';
...
export const sendCallStateSignal = (
  ...
  const callState = store.getState().call;
  const peer = (
    options?.peerXmppUsername ||
    callState.peerXmppUsername ||
    ''
  ).trim();
  if (!peer) {return;}
