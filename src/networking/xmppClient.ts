...
export class XmppClient implements XmppClientInterface {
    ...
    try {
      ...
      this.client.start().catch((error) => {
        if (error?.name === 'TimeoutError' || error?.message?.includes('Timeout')) {
          console.warn("XMPP connection timeout, will retry:", error?.message);
        } else {
      ...
      if (next === "active") {
        ...
        if (this.status !== "online") {
          ...
          this.reconnect().catch((error) => {
            console.error("Reconnect on app active failed:", error);
          });
        }
      } else {
  ...
  attachEventListeners() {
    ...
    this.client.on("error", (error) => {
      ...
      const isConnectionError =
        errorMessage.includes("ECONNERROR") ||
  ...
  async ensureConnected(timeout: number = 10000): Promise<void> {
    ...
    if (this.status === "offline" || this.status === "error") {
      ...
      const error = new Error(`Not connected. Status: ${this.status}`);
      error.name = "XMPPConnectionError";
      throw error;
    }
      ...
      await new Promise<void>((resolve, reject) => {
        const timeoutId = setTimeout(() => {
          const error = new Error(
            "Connection timeout while waiting for XMPP connection"
          );
          error.name = "XMPPConnectionTimeout";
          reject(error);
        }, timeout);
        ...
        const checkStatus = () => {
          if (this.status === "online") {
            ...
            clearTimeout(timeoutId);
            const error = new Error(
              `Connection error while waiting. Status: ${this.status}`
            );
            error.name = "XMPPConnectionError";
            reject(error);
          } else {
  ...
  async wrapWithConnectionCheck<T>(operation: () => Promise<T>): Promise<T> {
    try {
      ...
      // Create a proper error object to prevent unhandled promise rejections
      const wrappedError = error instanceof Error 
        ? error 
        : new Error(errorMessage);
      
      // Set error name for better error handling
      if (wrappedError instanceof Error && !wrappedError.name) {
        wrappedError.name = "XMPPOperationError";
      }

      // Don't throw if it's a connection timeout - let the caller handle it
      if (
