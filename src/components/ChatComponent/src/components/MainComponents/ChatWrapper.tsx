...
import {createIRoomRoom} from '../../helpers/createRoom';
...
const ChatWrapper: FC<ChatWrapperProps> = ({
  ...
  const initCustomRooms = (client: XmppClientInterface) => {
      ...
      if (!user.defaultWallet || user?.defaultWallet.walletAddress === '') {
        ...
        if (!client) {
          ...
          await initializeClient(
            ...
            if(!config?.customRooms?.rooms && config?.customRooms?.rooms.length === 0) {
