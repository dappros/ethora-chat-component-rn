...
const CustomMessageBubble = styled.View<{
  ...
  border-bottom-left-radius: ${({isUser, borderRadius}) => (isUser ? borderRadius || '10' : '0')}px;
  border-bottom-right-radius: ${({isUser, borderRadius}) => (isUser ? '0' : borderRadius ||  '10')}px;
  background-color: ${({
    isUser,
    deleted,
...
const Message: React.FC<MessageProps> = ({message, isUser, isReply}) => {
  ...
  const handleLongPress = () => {
    if (isLayoutReady && messageRef.current) {
        ...
        UIManager.measure(nodeHandle, (x, y, width, height, pageX, pageY) => {
          if (!isNaN(pageX) && !isNaN(pageY)) {
            ...
            setContextMenuPosition({
              x: pageX || 0,
              y: enoughSpaceBelow ? pageY + height : Math.max(0, pageY - 150),
            });
          }
        });
      }
      ...
      <View
        ...
        <TouchableWithoutFeedback
          ...
          <CustomMessageBubble
            ...
            backgroundMessageUser={config?.bubleMessage?.backgroundMessageUser}
            backgroundMessage={config?.bubleMessage?.backgroundMessage}
            borderRadius={config?.bubleMessage?.borderRadius}  
          >
            {!isUser && (
            ...
            {message?.isMediafile === 'true' && !message?.isDeleted ? (
              ...
              <>
                {message.isDeleted && message.id !== 'delimiter-new' ? (
                  ...
                  <CustomMessageText
                    isUser={isUser}
                    colorUser={config?.bubleMessage?.colorUser}
                    color={config?.bubleMessage?.color}>
                    {message.body}
                  </CustomMessageText>
                )}
              </>
            ...
            {!isUser && config?.enableTranslates && (
              <MessageTranslations
                message={message}
                config={config}
                langSource={config?.enableTranslates}
                isUser={isUser}
              />
            )}
