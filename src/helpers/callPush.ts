...
// while the socket is connected. On mobile the socket is dead whenever the OS
// backgrounds or kills the app, so the backend also delivers the call over
// push, in the same `data`-payload format as message pushes. This helper lets
// the push layer tell a call push apart from a chat push and pull the call
// fields out, tolerating the field-name variations backends tend to use.
