...
export const callStateHasLogData = (attrs: Record<string, any>): boolean => {
  ...
  const hasDuration =
    attrs.durationMs !== undefined &&
    attrs.durationMs !== null &&
    attrs.durationMs !== '';
 ...
 * the stanza arrives, which freezes the language: switch to French and
 ...
 * before this existed, and for the notification/room-list preview paths
 * that only have the raw message.
 */
export const formatCallLogLabel = (
  ...
  if (!callLog.durationMs || callLog.durationMs <= 0) {
    return callLog.direction === 'outgoing'
      ? t('call.noAnswer')
      : t('call.missed');
  }
