...
- **Dark theme — `config.dark` + `config.darkColors`.** One flag switches the whole component (room list, both headers, conversation, bubbles, composer, attach sheet, context menus, every modal and sheet) to a dark palette. Colours live in one place now: `src/theme/theme.ts` holds `LIGHT_THEME` (the previously hard-coded values, so light rendering is unchanged) and `DARK_THEME`; `resolveTheme(config)` merges `darkColors` over the dark defaults, so a host passing `{ dark: true }` alone gets the built-in palette and `{ dark: true, darkColors: { primary, surface, … } }` overrides only what it names. `darkColors.primary` also drives `icon` / `senderName` / `dateLabel` unless set, mirroring `colors.primary`. In dark mode the light-only knobs (`colors` except `colors.avatar`, `messageColor`, `backgroundChat.color`) are not applied — they were tuned for white surfaces. Components read the palette through `useTheme()` (which works without a redux Provider, so leaf components stay renderable in isolation) or `({ theme }) => theme.x` in styled templates via a `ThemeProvider` mounted in `ReduxWrapper`; the existing `getIconColor` / `getSenderNameColor` / `getDateLabelColor` / `getChatBackgroundColor` helpers are theme-aware. `LIGHT_THEME`, `DARK_THEME`, `resolveTheme`, `isDarkTheme`, `useTheme` and the `ChatTheme*` types are exported from the package root so a host can paint its own chrome (status bar: `resolveTheme(config).statusBarStyle`). Testbed: a "Dark theme" toggle on the Setup tab. Tests in `theme.test.ts`.
