...
- **`config.inputDockPaddingBottom`** - controls the input dock's bottom padding directly, used verbatim on both platforms when a number is given (0 allowed), instead of the built-in default (iOS safe-area inset, 12dp on Android). Customer-reported #44.
...
- **Profile header polish (both the chat and the user profile).** The back / "…" row is taller and sits further from the top edge, and its buttons are inset 20 pt so a button's centre line matches the collapsed avatar's — they used to sit closer to the edge than the avatar below them. The glyphs inside the hero's round buttons always paint white now (several SDK icons ignore their `color` prop and paint white regardless, so tinting the rest to the title colour left a mix of black and white in one row); on a light fill the circle darkens instead of the glyph. And on the user profile the "…" menu appears once the header is collapsed, carrying exactly the same actions the hero's buttons do — collapsed, those buttons are off-screen and were unreachable.

- **Settings screen restyled.** Manage Data and Visibility are separate white cards on a light grey ground — rounded, shadowed, each its own tap target with a chevron — instead of two rows sharing one bordered block. Their labels come from the i18n table now (`settings.manageData.title` / `settings.visibility.title`), so the misspelled hardcoded "Visiblility" is gone. The shared full-screen modal header (Settings, Manage Data, Visibility, the shares screens…) got the same card treatment as the room-list header: rounded bottom corners and a soft shadow in place of its hairline rule. Tests in `userSettingsScreen.test.tsx`.

- **Tapping a room's avatar opens the chat.** Only the text half of a row did: `ProfileImagePlaceholder` wraps its avatar in a Touchable, which became the responder and swallowed the tap. The avatar is now wrapped in a `pointerEvents: 'none'` view, so the whole row is one target. Row separators also darkened to `#C6CFDA` — the old `#F0F0F0` was lighter than the list's new ground and had disappeared into it.
- **`config.headerMenu` accepts `true`.** The room-list burger (left of the header) previously required a host callback, so an app that just wanted the SDK's own menu had nothing to pass. `headerMenu: true` now shows the button and opens that menu — the same sheet the header avatar opens; a function still means the host drives its own drawer, and `disableRoomMenu` hides the button either way. Enabled in the testbed app. Tests in `roomListSearchOverlay.test.tsx`.

- **Room list, headers and composer restyled.** The room list's ground is now `#E8EDF2` (the search field keeps its white card on top of it, and a host's `roomListStyles` still overrides it). The search strip floats over the list on a transparent background — the rooms scroll underneath it — while staying fixed rather than scrolling away; the list's top padding is measured from the strip, so it always starts clear of the field. Both headers — the room list's and the in-chat one — became white cards with 20 pt rounded bottom corners and a soft drop shadow in place of their hairline bottom rule, and the composer mirrors it: rounded top corners with the shadow cast upwards instead of a rule that read as a seam. For those corners to be visible at all, the surface behind them had to stop being white: the chat container (and the empty-room placeholder) now paint the conversation's own colour through the new `getChatBackgroundColor` helper — which is also what the message list uses, so the two can't drift — and the rounded top plus upward shadow live on the composer's dock alone, with the inner container transparent: on the inner view too, its shadow fell onto the dock as a faint seam that read as two different whites, and an opaque inner view simply painted over the dock's rounded corners (a border radius doesn't clip children without `overflow: hidden`, which would clip the shadow as well).

- **The room-list header menu is a bottom sheet.** New Chat / Profile / Settings (and Sign out when the host enables it) used to slide in as a full-height drawer covering 70% of the screen from the right; they now come up from the bottom in a sheet exactly as tall as its rows, with a grab handle and fully rounded corners, inset 3 pt all round so the dimmed screen shows as a thin line around it — reachable by thumb, and the list stays visible behind it. Like the create-chat dialog it presents through a real RN `<Modal>`; as an in-tree overlay it was bounded by the room list's own container, which left a pale strip of app background under the sheet and stopped the dim short of the host's chrome. It can be pulled down to dismiss: the grab area at the top claims the gesture on touch-down, and the body claims it in the capture phase, because a drag starting on a row hands the responder to that row's Touchable on touch-down and a non-capturing responder is never consulted again (the same fix went into the attach sheet's body). The drag offset is a second `translateY` entry rather than an `Animated.add` around the parent's natively-driven value, and is handed to the native driver when the sheet opens — a transform that mixes a native node with a JS-only one drops the JS updates, which reads as a drag that moves nothing. and it is unmounted while closed rather than parked off-screen, where its shadow bled along the bottom edge. The row icons take `colors.primary`. Same testIDs, same logout flow.
- **The testbed app no longer pads the bottom safe area** (`AppLoginChatsRn` passes `edges={['top','left','right']}`): the blank white band it reserved sat under the room list and under every sheet the chat presents. Content now runs under the home indicator, the way a messenger does.
- **Drag-to-dismiss thresholds now live in `helpers/sheetGestures.ts`** and are shared by the attach sheet, the menu sheet and anything added later, so a pull feels the same everywhere (the attach sheet re-exports them, so its tests are unchanged).
- **The room-list header avatar is a little bigger** (30 → 36 pt), with the header's side slots widened to match.

- **"Create new chat" redesigned.** Title, a round 72 pt picker (camera glyph, tap to pick, long-press to clear), a "Chat name" field, a multi-line "Description (optional)" field, and Cancel / Create side by side. The private-chat toggle from the design is deliberately left out.
  - It presents itself through a real RN `<Modal>` (`statusBarTranslucent`), so it covers the whole window with the design's opaque light ground (the chat list behind is hidden, not showing through a translucent dim). The old in-tree overlay stopped at the SDK's own frame and left the host app's chrome — tab bars, headers — undimmed above it. `Modal/Modal.tsx` skips its shared backdrop for this one so nothing dims twice.
  - Every accent (the picture circle, the Create button) uses the configured `colors.primary` instead of the hardcoded `#0052CD` blue. Validation, image upload and both creation paths (REST `postRoom` and the XMPP `createRoomStanza` fallback) are unchanged. Tests in `newChatModal.test.tsx`.

- **User profile screen rebuilt on the same header as the chat profile.** The picture becomes a full-bleed hero (or a flat colour with two initials) with the same parallax, collapse-into-a-bar and frosted blur; the collapsing header, the "…" menu and the over-scroll behaviour now live in shared `ProfileHeader` / `ProfileMenu` components that both screens use.
  - Actions differ by whose profile it is: **your own** gets Account (the existing settings modal), Share, Edit and Log out as round buttons, plus a **Leave** button at the right end of the name row that runs the SDK's full logout teardown (`runLogoutFlow` — the same confirm copy, callbacks and cleanup as the room-list menu's Sign out). **Someone else's** gets Message (creates the private room, as before) and Share. Nothing is left behind a "…" overflow, so this screen has no menu button at all. Share opens the system share sheet with the name and user id, behind a new `ShareIcon` (three linked nodes) — the `SendItemIcon` it borrowed at first is a textured token badge with a hardcoded blue outline that ignored the button's tint. Per the design, the counters and the QR button are gone; Block is not offered because the SDK has no blocking anywhere (no API, no XMPP privacy lists — `BlockedUsersModal` is a static placeholder).
  - **About** shows the user's description, with "No description" as the fallback.
  - **Tabs on your own profile**: Language, Media and Documents. Language lists `LANGUAGE_OPTIONS` with the active one checked and writes the same `langSource` the in-chat globe uses. Media and Documents come from one `GET /v2/files/` on open, split by mime type (with a filename-extension fallback for deployments that store no mimetype); a tab only appears when it has something in it, and tapping a file opens the existing file-preview modal. Another user's profile has no tabs at all — that endpoint is scoped to the JWT owner, so it would show *your* files on *their* page.
  - New `getUserFiles()` / `isMediaFile()` in `user.api.ts` read both the v2 `{ results, pagination }` shape and the legacy `{ items }` one, and fall through the field aliases (`location` / `locations[]`, `originalname` / `filename`) different deployments use. Tests in `userProfileScreen.test.tsx`.

- **Chat profile screen rebuilt around the chat photo.** The picture is now a full-bleed hero with the chat name, member count and round action buttons over it; scrolling the member list collapses it into a compact bar (small avatar + name + member count) that fades in over the same photo, and pulling down zooms the picture. Both the parallax and the collapse run off a single `Animated.Value` fed by `Animated.event(..., { useNativeDriver: true })`, so the whole transition stays on the UI thread. A chat with no picture gets a solid background (the per-chat avatar colour, config `colors.avatar`/primary) with two uppercase initials, same rule people's avatars use.
  - Below the hero: the description and chat-type cards, an **Add Members** row (moderators of group chats only — it opens the existing `SelectUsersModal`, which now takes an optional `trigger` render prop), and the member list with the owner/moderator role on the right and a red `banned` label for banned members. Moderators get a per-row remove button (confirmed with a native alert) in place of the old three-dot menu, whose "Appoint as an admin" and "Unban" entries did nothing but open unrelated modals.
  - Actions: **Leave** (an unavailable presence via `leaveTheRoomStanza`, then the room is dropped from the store, which closes the screen) and **Report**, plus a "…" menu with **Edit** (change the chat picture), **Remove photo**, **Report** and **Delete and Leave** — every entry gated on what the signed-in user can actually do. Report opens a new sheet with the seven `report.category.*` options the i18n table has always carried and posts them to `POST /v1/chats/reports/{chatName}`.
  - The collapsed bar follows the design's third screen: a buttons row (back, "…") with the 48 pt round avatar, chat name and member count on their own row below it, over the frosted picture.
  - The header frosts as it collapses: an `expo-blur` `BlurView` (intensity 92, with a 0.28 dim over it) on the picture, its opacity driven by the same scroll value. `expo-blur` is a new OPTIONAL peer dependency (`>=13.0.0`, dev-pinned to `~57.0.2`). Having the JS package is not enough — the native view has to be in the binary, and an app that installed it without rebuilding renders a red "Unimplemented component: ExpoBlurView" box — so the layer is gated on `requireNativeModule('ExpoBlur')` succeeding and falls back to a plain dim otherwise.
  - The collapsed state is always reachable: the scroll content gets a `minHeight` of the measured viewport plus the hero-to-bar distance, so a chat with two members can still be pulled up instead of springing back.
  - **Search** sits to the left of Leave (the action row is centred: Search / Leave / Report) and filters the member list by full name. Opening it from an expanded header collapses the picture out of the way; dismissing search — or just putting the keyboard away — puts the header back exactly as it was. From an already-collapsed header it only opens the field, and since the hero's button is off-screen there, **Search joins the "…" menu** while collapsed.
  - Over-scrolling downwards stretches the picture instead of painting a second copy of it: the backdrop reaches 400 pt above the hero, clips itself, and scales about its own bottom edge by exactly the amount the over-scroll exposed. The parallax is clamped at zero for negative offsets — left extending, it lifted the picture off that bottom edge and showed a band of backdrop colour beneath it.
  - The "…" menu is now anchored to its own button and unfolds out of it — it measures the button with `measureInWindow` and scales the card up from its top-right corner. The shared `DropdownMenu` pins its card at a hard-coded `top: 95`, which on this screen landed over the host app's tab bar with no relation to the button.
  - Without a picture the hero is ONE flat colour (`colors.avatar`, else the primary) with no scrims — the two stacked scrims that used to sit on it showed their seam as two different greens. Over a photo the white text sits on a gradient that fades in from nothing over the bottom 220 pt — stacked from 16 alpha-ramped bands, since the package carries no gradient dependency. (A single flat overlay has a hard top edge, which read as the bottom half of the picture being darkened.) The initials sit in the upper half so they never collide with the chat name.
  - Removed per the design: the QR button (it had no handler behind it — `setVisible` was dead state) and the favourites star, which this SDK never had. `typography.profile.screenTitle` now styles the chat name in the collapsed bar; `typography.profile.title` the name over the photo. New i18n keys (`modal.chatProfile.addMembers`, `.membersLabel`, `.membersUnavailable`, `.removeMember`, `.removePhoto`, `action.edit`, `action.deleteAndLeave`, `status.banned`) are translated in all six built-in locales. `LeaveIcon`, `ReportIcon` and `BackIcon` now honour their `color` prop (their strokes were hardcoded); defaults are unchanged, so every existing call site looks the same. Tests in `chatProfileScreen.test.tsx`.

- **Attach sheet reworked into a Slack-style media picker.** Tapping the paperclip now opens on a "Photos & Videos" section: a horizontal strip with a camera tile first, then the 12 most recent photos read through `expo-media-library` (optional peer). Tapping a thumbnail attaches it directly, skipping the system picker; "View Library" opens the full `expo-image-picker` gallery; "Upload a File" below the divider is the document route. Every existing capability (take a photo, pick from the gallery, pick a file) is still one tap away, and picks keep going through the same deferred-until-dismissed handler as before. iOS `ph://` asset ids are resolved via `getAssetInfoAsync` to an on-disk uri, MIME is derived from the filename extension, images still get the HEIC→JPEG pass, and file size is probed so the 50 MB cap is enforced client-side. The Cancel button is gone (backdrop tap, drag-down and the back gesture all close the sheet), as are the per-row hint lines. New typography key `typography.attachSheet.viewLibrary`; `rowHint` and `cancelButton` are deprecated. Tests in `attachSheetRecents.test.tsx`.
  - Library permission is requested by `SendInput` **before** the sheet opens, not from inside it: iOS cannot present the permission alert while an RN `<Modal>` owns the screen (the same UIKit limitation that used to swallow the pickers), so asking from within resolved to a silent "denied" and left the strip permanently empty. Only the first paperclip tap of a session goes through the check. Without access the strip is simply the camera tile — there is no nagging "allow access" placeholder.
  - Thumbnails are resolved to real `file://` paths before they are rendered. On iOS the library hands out `ph://` identifiers, and React Native has had no PhotoLibrary image loader since CameraRoll left core — the one `expo-media-library` ships isn't picked up under the New Architecture, so every thumbnail redboxed with `No suitable image URL loader found for ph://…` and the second open crawled under the pile of them. `getAssetInfoAsync(asset, { shouldDownloadFromNetwork: false })` maps each asset to its on-disk original (Android's uri is already a file path and skips the call entirely); iCloud-only assets resolve to nothing and are dropped from the strip rather than being pulled over the network to draw an 88 pt tile. The tapped photo is then handed over with no further native round-trip. The strip lists photos only — `<Image>` cannot decode a video file, and poster frames would mean another native dependency; videos stay one tap away behind "View Library".
  - Thumbnails are decoded at 2× their display size (explicit `width`/`height` on the image `source`, `resizeMethod="resize"` on Android) instead of at full sensor resolution, the strip loads 250 ms after the sheet opens rather than during the entry animation, results are cached for 15 s across open/close, and `expo-media-library` is now required lazily on first open instead of at chat-render time — the sheet no longer stutters when it is opened and closed repeatedly.
- **The attach sheet's grabber now drags.** Two `PanResponder`s share one follow-the-finger animation: the grab handle claims the gesture on touch-down (so a pull on the bar always drags), while the sheet body claims only once the movement is clearly vertical, leaving row taps and the horizontal strip scroll alone. Release past 90 pt (or a flick over 0.8 px/ms) closes the sheet with the normal slide-down animation from wherever the finger let go; anything shorter springs back.

- **`config.logout` — built-in "Sign out" item in the room-list header menu.** `{ enabled, label?, confirm?, onBeforeLogout?, onAfterLogout? }`. Rendered last (after New Chat / Profile / Settings) with a logout icon tinted `colors.primary`; hidden unless `enabled: true`, so existing menus are unchanged. Tap flow: close drawer → native confirm (`confirm` defaults to `true`; pass an object to customise title/message/button copy, `false` to skip) → `await onBeforeLogout?.()` (return `false` to cancel) → `await logoutService.performLogout()` (full teardown: XMPP, redux, persisted slices, AsyncStorage, REST cache) → `await onAfterLogout?.()` for the host's own session/navigation logout. Host callback errors are caught and logged via `console.warn` — a throwing `onBeforeLogout` cancels, a throwing `onAfterLogout` is swallowed after the teardown has landed. Documented in README ("Logging out") and `instructions.md`. Tests in `headerRoomListMenuLogout.test.tsx`.
- **`useLogout` is now exported from the package root** (previously only `logoutService`), so a host can build its own button with the same awaitable teardown.

### Fixed

- **Leaving a room while scrolled up marked genuinely-unread messages as read, locally and on the server (customer #42, a regression of #33 reintroduced by #38).** #38 anchored the read marker to the newest server-acknowledged message instead of `Date.now()`, but every "leaving this room" path (unmount, AppState background, the host's `isVisible` prop going false, the live `advance()` effect, `useChatRoomFocus`) then stamped that newest message unconditionally, regardless of how far the user had actually scrolled - and because the private-store merge is forward-only, a later correct (smaller) marker could never overwrite the wrong one. A per-room read boundary (`rooms.readBoundaries`, populated by `MessageList` the moment the user scrolls away from the bottom) is now the single source every one of those paths consults via the new `getReadMarkerTimestamp` helper, and is carried through to the server write as `visibleRoomTs`. Also fixed: the "New messages" divider landed one message too high (above the last read message instead of directly below it). The divider was positioned by comparing `msg.date` strings against `new Date(marker).toString()`, and `Date.prototype.toString()` has second precision - the marker was silently truncated down to the whole second, so the very message it points at compared as "newer" and fell on the unread side, disagreeing by one with the unread count (which compares exact milliseconds). `insertMessageWithDelimiter` now compares numerically, on the same server id-encoded `msgSortableMs` source the unread count itself uses, and also accepts a raw numeric marker (`new Date("1700000000000")` is an Invalid Date, so a numeric-string marker used to make every comparison false).
- **Push notifications never arrived for users who only ever signed in through this SDK.** The `initBeforeLoad` bootstrap in `XmppProvider` re-joined rooms with presence but never sent the MucSub (`urn:xmpp:mucsub:0`) subscription — only the legacy `ChatWrapper` path did, via `initRoomsPresence`. ejabberd decides whom to push for offline messages by MucSub subscriptions, so such users had a registered device token and still got nothing ("no MucSub subscription" server-side). The bootstrap and the reconnect `onOnline` handler now subscribe every room in the store after the presence pass, regardless of how that pass ended; `pushSubscriptionService` still dedupes rooms already subscribed on the device.
- **MucSub subscriptions are sent in parallel batches and carry unique IQ ids.** The pass used to be strictly sequential (await + 100 ms pause per room, 5 s timeout each), so a user with a few dozen rooms needed minutes and a backgrounded socket died long before the end, leaving most rooms unsubscribed. Ids were `newSubscription:<Date.now()>`, which in a batch made every request resolve on the first reply. A refused subscription now logs the server's condition (`registration-required`, `forbidden`, …) so room-config / affiliation problems are visible instead of silent.
- **The XMPP stream is closed as soon as the app goes to the background** (`XmppClient.suspend()`), and reopened on foreground (`resume()` → immediate reconnect). Android keeps a backgrounded WebSocket alive for minutes; while it lived, ejabberd still saw the user online and routed new messages into the socket instead of sending a push — so only the very first push after a cold start ever arrived, and nothing after the app had been opened and backgrounded again. The suspend is immediate, not deferred: RN pauses JS timers while an Android app is backgrounded. A reconnect that was in flight when the app got backgrounded closes its fresh socket at once, and `resume()` skips the 3 s `stop()` timeout on the already-closed client. Opt out with `xmppSettings.keepAliveInBackground: true`.
- **`allRoomPresences` reported a failure on every bootstrap.** `presenceInRoom` schedules its resolve `delay` ms (2000) after the room's reply and races a 2000 ms timeout that rejected with `undefined` — the timeout won, so a join that had succeeded was logged as `allRoomPresences failed`. The timeout no longer rejects once the reply has been seen.

- **The signed-in user's description was thrown away on every login.** `unpackAndTransform` (the shaper `setUser` runs every payload through) hardcoded `description: ''`, so the profile's About section could never show your own bio no matter what the server returned. It now carries `input?.description` through.
- **Avatars for names that start with a digit rendered blank.** `ProfileImagePlaceholder` picked initials with `/^[a-zA-Zа-яА-ЯёЁ]$/`, so a member called "2Test 2User" matched neither letter and got an empty coloured circle. The class is now `/^[\p{L}\p{N}]$/u` — every script's letters plus digits.
- **"Save to gallery" in the file preview threw instead of saving (SDK 54+).** `expo-media-library` 17 moved its function API behind the `expo-media-library/legacy` subpath and turned the root's same-named exports into stubs that `console.warn` and then `throw` ("Method `saveToLibraryAsync` imported from "expo-media-library" is deprecated"). Since the names still exist and still type-check, the breakage was invisible: every save attempt ended in the generic "Failed to save the file" alert, and each one also queued a LogBox warning whose stack symbolication stalls the JS thread in dev builds. All call sites now go through the new `helpers/mediaLibraryRuntime.ts`, which prefers `/legacy`, falls back to the root export for older versions, returns `null` when the optional peer isn't installed, and memoises the result. Tests in `mediaLibraryRuntime.test.ts`.
- **iOS: attach-sheet rows (Take photo / Photo or video / Document) did nothing, and the next tap threw `PickingInProgressException: Different document picking in progress`.** The picker handler was invoked from the exit-animation completion callback in the same tick as `setMounted(false)` — but that only *starts* RN `<Modal>`'s native dismiss, so the sheet's UIViewController was still on screen when `expo-image-picker` / `expo-document-picker` called `currentViewController().present(picker)`. UIKit silently dropped the present, the promise never resolved, and expo-document-picker kept its `pickingContext` stuck. `AttachSheet` now fires the pending handler from `<Modal onDismiss>` on iOS (the only signal that the view controller is really gone); Android, where `onDismiss` never fires, keeps running it from the animation callback. The handler is nulled before invocation so it runs at most once, and an interrupted animation (`finished: false`) still does nothing. Tests in `attachSheetPendingHandler.test.tsx`. On iOS, picker rows also skip the sheet's slide-down (the modal is hidden immediately, so `onDismiss` arrives ~50 ms after the tap instead of ~300 ms); Cancel and backdrop taps keep the animated close. The old 350 ms iOS `setTimeout` in `SendInput.handleFileSelection` that tried to paper over the same race is removed — the Document picker now opens as soon as the sheet is gone instead of noticeably later.

### Security

- **Chat tokens, the XMPP password, and message history are no longer stored in plaintext AsyncStorage.** All four bearer credentials on `User` (REST access + refresh token, XMPP password, secure-media fileToken) now go through the platform Keychain (iOS) / Keystore-backed EncryptedSharedPreferences (Android) via the optional `expo-secure-store` peer dependency, instead of sitting in AsyncStorage's plain SQLite/plist storage. The persisted message-history and room-list cache (`roomStore/persistence.ts`) is now AES-256-CBC encrypted at rest, with the symmetric key itself held in that same secure store — envelope encryption, since bulk history is too large (and unbounded) for direct Keychain storage. `expo-secure-store` is optional (mirrors `expo-video`/`expo-audio` etc.): a host that hasn't added it falls back to namespaced-but-still-unencrypted AsyncStorage with one logged warning, so nothing breaks for existing consumers, but the secure path is what ships by default for any host on the standard Expo dependency set.
  - New `helpers/secureStoreRuntime.ts` (lazy `expo-secure-store` bridge, same pattern as `livekitRuntime.ts`), `helpers/secureKeyValue.ts` (get/set/remove with the AsyncStorage fallback), `helpers/secureUserStorage.ts` (splits `User` into a secret-free AsyncStorage profile + four Keychain-backed secret keys behind the same `{get,set,remove}` shape the old `asyncLocalStorage<User>(ETHORA_USER)` had, so every call site — `chatSettingsSlice`, `authRefresh`, `resolveInitBeforeLoadUser`, `xmppProvider`, `ensureScopedChatCache`, `useLogout`, the testbed's `Login.tsx` — swapped in without changing behavior), `helpers/persistCrypto.ts` (AES-256-CBC envelope encryption for the bulk cache, random IV per write via `react-native-get-random-values`, key generated once and cached).
  - A pre-encryption plaintext cache from an older install decrypts to `null` (not a crash) and is treated as an empty cache — the app just re-hydrates from the server, same as any cold start.
  - Verified on-device (iOS Simulator, real `expo-secure-store` linked via CocoaPods, not the jest mock): no fallback warning logged through a full login → XMPP connect → room list → message render cycle; the raw `@ethora/chat-component-user` AsyncStorage blob has all four secret fields as empty strings with no JWT-shaped substring anywhere in it; the raw `@ethora/persist:rooms` blob is a genuine `{v,iv,ct}` AES envelope containing none of the actual on-screen message text; logout leaves only the non-secret tenant-scope key on disk. 18 new unit tests (`secureStorage.test.ts`) plus updated coverage in `authRefresh.test.ts`, `resolveInitBeforeLoadUser.test.ts`, and both `persistence*.test.ts` suites (760 total, all green); `tsc` clean; real `bob build` (the exact step `npm publish`'s `prepack` runs) succeeds.
...
- **The input dock's bottom padding now defaults to `0` when `disableKeyboardAvoidingView` is set** (it used to be the iOS safe-area inset / 12dp on Android regardless). A host that owns keyboard layout typically already places the chat above its own chrome, e.g. a tab bar, and the built-in padding added a visible extra gap there (customer-reported #44). **Behaviour change for existing consumers of `disableKeyboardAvoidingView`:** if your chat does sit at the very bottom of the screen and you want the old inset back, set `config.inputDockPaddingBottom` explicitly. Hosts that do not set `disableKeyboardAvoidingView` are unaffected.
